...
const GTC = ({ isOpen, onClose }) => {
  ...
  const handleDownload = () => {
    ...
    const titleLines = doc.splitTextToSize('Allgemeine Geschäftsbedingungen', textWidth);
    doc.text(titleLines, marginLeft, y);
    y += titleLines.length * 24 * 0.5 + 10;

    const contentArray = parseHtmlToPdf(content || '<p>Kein Inhalt verfügbar.</p>');
    ...
    if (isOpen) {
      const fetchGTC = async () => {
        try {
          ...
          setContent(gtcSection?.content.description || '<p>Kein Inhalt verfügbar.</p>');
        } catch (err) {
          console.error('Error fetching GTC:', err);
          setError('Failed to load GTC content.');
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div
        ...
        role="dialog"
        aria-labelledby="gtc-title"
      >
        <div className="flex justify-between items-center mb-6">
          <h3 id="gtc-title" className="text-3xl font-semibold text-[#074a5b]">
          ...
          <div className="flex items-center gap-4">
            <button
              ...
              style={{ fontFamily: "'Comic Sans MS', 'Comic Neue'" }}
              aria-label="Download General Terms & Conditions as PDF"
            >
            ...
            <button
              ...
              aria-label="Close General Terms & Conditions"
            >
              <X size={24} />
            </button>
          </div>
